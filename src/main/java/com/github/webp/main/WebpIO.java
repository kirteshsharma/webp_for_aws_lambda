...
public class WebpIO {
  ...
  public static void toNormalImage(File src, File dest) {
    String command = CMD_DIR + (dest.getName().endsWith(".gif") ? "/gif2webp" : "/dwebp ") + src.getPath() + " -o "
        + dest.getPath();
  ...
  public static void toWEBP(File src, File dest) {
    try {
      String command = CMD_DIR + (src.getName().endsWith(".gif") ? "/gif2webp " : "/cwebp ") + src.getPath() + " -o "
          + dest.getPath();
      System.out.println("Execute: " + command);
      String output = executeCommand(command);
      if (!"".equals(output)) {
